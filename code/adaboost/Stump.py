...
class Stump:
    ...
    def calculate_error_rate(X, Y, W, t, feature_idx, high_one=True):
        ...
        for i in range(X.shape[0]):
            e += W[i] * 1 \
              if ((X[i, feature_idx] - t) * Y[i, 0] < 0 if high_one else (X[i, feature_idx] - t) * Y[i, 0] > 0) \
              else 0
        return e[0]
     
    def train(self, X, Y, W, step=1.0):
        """
        Train the stump and return the error (as in a boosting tree)
        """
        e = 1.0 # error rate
        for f_idx in range(X.shape[1]):
            ...
            while t <= high + step / 2:
                ...
                t += step
        self.f = lambda x: 1 \
            if (self.high_one and x[self.feature_idx] > self.threshold) or \
                (not self.high_one and x[self.feature_idx] < self.threshold)  \
                    else -1
        return e